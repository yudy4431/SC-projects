    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(YEARS)-1):
            ...
            if int(rank) == MAX_RANK and int(rank2 != MAX_RANK):
                canvas.create_line(x1, CANVAS_HEIGHT-GRAPH_MARGIN_SIZE,
                                   x2, GRAPH_MARGIN_SIZE + int(rank2) * ratio,
                                   fill=color,
                                   width=LINE_WIDTH)
                ...
                canvas.create_oval(x1-R, CANVAS_HEIGHT-GRAPH_MARGIN_SIZE-R, x1+R, CANVAS_HEIGHT-GRAPH_MARGIN_SIZE+R,
                                   fill=color)

            elif int(rank2) == MAX_RANK and int(rank) != MAX_RANK:
                canvas.create_line(x1, GRAPH_MARGIN_SIZE + int(rank) * ratio,
                                   x2, CANVAS_HEIGHT-GRAPH_MARGIN_SIZE,
                                   fill=color,
                                   width=LINE_WIDTH)
                canvas.create_text(x1 + TEXT_DX, GRAPH_MARGIN_SIZE + int(rank) * ratio,
                                   text=lookup_names[i]+' '+rank, anchor=tkinter.SW, font='modern 12', fill=color)

                canvas.create_oval(x1-R, GRAPH_MARGIN_SIZE + int(rank) * ratio-R, x1+R,
                                   GRAPH_MARGIN_SIZE + int(rank) * ratio+R, fill=color)
                ...
                canvas.create_oval(x1-R, GRAPH_MARGIN_SIZE+int(rank)*ratio-R, x1+R, GRAPH_MARGIN_SIZE+int(rank)*ratio+R,
                                   fill=color)
